      ...
      if (script.coverImageUrl) {
        ...
        if (!fs.existsSync(filePath)) {
          ...
          try {
            ...
            console.log(`   ❌ ERROR reading cover ${filename}: ${error.message}`);
          }
        }
      }
    }
    ...
    if (issues.length === 0) {
      ...
      if (missingFiles.length > 0) {
        ...
        missingFiles.forEach(issue => {
          console.log(`   • ${issue.filename} (${issue.expectedSize || 'unknown'} bytes expected)`);
          if (issue.databaseRecord && issue.databaseRecord.script && issue.databaseRecord.script.title) {
            console.log(`     Script: "${issue.databaseRecord.script.title}"`);
          } else if (issue.databaseRecord && issue.databaseRecord.scriptTitle) {
            console.log(`     Script: "${issue.databaseRecord.scriptTitle}"`);
          }
        });
      }
      ...
      if (corruptedFiles.length > 0) {
        ...
        corruptedFiles.forEach(issue => {
          ...
          console.log(`     Expected: ${issue.expectedSize || 'unknown'} bytes`);
      ...
      console.log('\n💡 RECOMMENDED ACTIONS:');
      console.log('   1. Run: node scripts/file-system-repair.js');
