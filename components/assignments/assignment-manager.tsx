...
import { Search, Plus, Filter, Users, Clock, CheckCircle, AlertTriangle } from 'lucide-react'
...
export function AssignmentManager({ scriptId, userId, showHeader = true, readOnly = false }: AssignmentManagerProps) {
  ...
  const canAssign = !isSessionLoading && (session?.user?.role === 'ADMIN' || session?.user?.role === 'EXECUTIVE' || session?.user?.role === 'READER') && !readOnly
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              {scriptId ? 'Script Assignments' : userId ? 'My Assignments' : 'All Assignments'}
            </h2>
            <p className="text-gray-600 mt-1">
              {stats.total} assignment{stats.total !== 1 ? 's' : ''} total
            </p>
          </div>
          {canAssign && (
            <AssignmentForm 
      ...
      {/* Always show assignment form for management users when they can assign, even without header */}
      {!showHeader && canAssign && (
      ...
      {groupedAssignments.OVERDUE.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold text-red-900">Overdue Assignments</h3>
            <Badge variant="destructive">{groupedAssignments.OVERDUE.length}</Badge>
          </div>
        ...
        return (
          <div key={status} className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="secondary">{statusAssignments.length}</Badge>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <AnimatePresence>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            {searchTerm || statusFilter !== 'ALL' || priorityFilter !== 'ALL'
              ...
              : scriptId 
                ? 'This script has no assignments yet.'
